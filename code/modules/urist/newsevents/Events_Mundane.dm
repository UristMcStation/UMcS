/datum/round_event_control/mundane_news
	name = "Mundane News"
	...
	if(!newMsg.body)
		...
		switch(event_type)
			...
			if(SONG_DEBUT)
				...
				[pick("roses","three stars","starships","nanobots","cyborgs","Unathi","Carp")] \
				[pick("on Venus","on New Earth","on Ryclies II","in my hand","slip through my fingers","die for you","sing your heart out","fly away")]' \
			...
			if(MOVIE_RELEASE)
				...
				[pick("Deadly","The last","Lost","Dead")] [pick("Starships","Warriors","outcasts","Commandoes","Unathi","Carp")] \
				[pick("of","from","raid","go hunting on","visit","ravage","pillage","destroy")] \
				[pick("Ryclies I","New Earth","Mars","Antioch V","New Eden","the Void","the Edge of Space")]'.\
...
/datum/round_event/trivial_news/announce()
	...
	"[pick("Trader","Spacer")] gets tattoo of Tau Ceti on chest '[pick("CentComm","star","starship","asteroid")] tickles most'.",\
	"Cyborg marries computer; wedding attended by 100 modems.",\
	...
	"Terran Scientist Discovers Abacus Can Be Used To Dry Towels",\
	...
	"Survey: 3 Out Of 5 crewmembers Loathe Modern Art",\
	"Terran Scientist Discovers Gravity While Falling Down Stairs",\
	"Shipment Of Apples Overturns, [affected_dest.name] Diner Offers Applesauce Special",\
	"Spotted Owl Spotted on [affected_dest.name]",\
	"Humans Everywhere Agree: Dead Space Carp are Good Space Carp",\
