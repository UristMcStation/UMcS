	...
	if(useMS)
		...
		if(prob(2))
			...
			switch(pick(1,2,3,4,5,6,7))
				...
				if(2)
					sender = pick(300;"QuickDatingSystem",200;"Find your russian bride",50;"Terran beauties are waiting",50;"Find your secret sexy crush",50;"Beautiful unathi brides")
					message = pick("Your profile caught my attention and I wanted to write and say hello (QuickDating).",\
					"If you will write to me on my email [pick(first_names_female)]@[pick(last_names)].[pick("ru","ck","tc","ur","nt")] I shall necessarily send you a photo (QuickDating).",\
				...
				if(5)
					...
					message = pick("YOUR FUND HAS BEEN MOVED TO [pick("Ryclies","Antioch","New Eden","Andromeda","Gruis","Corona","Aquila","ARES","Asellus")] DEVELOPMENTARY BANK FOR ONWARD REMITTANCE.",\
				...
				if(6)
					sender = pick("NanoTrasen Morale Divison","Feeling Lonely?","Bored?","www.wetunathi.nt")
					message = pick("The NanoTrasen Morale Division wishes to provide you with quality entertainment sites.",\
					"WetUnathi.nt is a xenophillic website endorsed by NT for the use of male crewmembers among it's many stations and outposts.",\
					"WetUnathi.nt only provides the higest quality of male entertaiment to NanoTrasen Employees.",\
